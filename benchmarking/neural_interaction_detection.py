...
class NeuralInteractionDetectionExplainerTF():
    def __init__(self, model):
        ...
        """
        self.model = model
        self.first_layer_weight = self.model.layers[0].weights[0]
        ...
        if inputs is not None:
            ...
            for k in iterable:
                ...
                for i in range(num_features):
                    for j in range(i + 1, num_features):
                        ...
                        interaction_strength = np.abs(np.minimum(weight_times_input_i,
            ...
            for i in iterable:
                for j in range(i + 1, num_features):
                    ...
                    interaction_strength = np.abs(np.minimum(weight_i, weight_j)) * unit_influence
